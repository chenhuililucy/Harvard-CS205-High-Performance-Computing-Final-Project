...
double OMP_Eigen(cv::Mat x, cv::Mat base, int coeff_count)
{
	...
	double sum = 0.0;  // Variable to store the sum of all values
	...
	double average = sum / result.size();  // Calculate the average
...
double OMP(cv::Mat x, cv::Mat base, int coeff_count)
{
	vector<pair<int, double>> result;
