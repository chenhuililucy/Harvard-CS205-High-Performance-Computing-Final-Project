...
vector<DataPoint> GetTrainingData(const TrainingParameters &tp)
{
	...
	/*
	if (process_rank==0){
	    string filename = "main_train__result.yml";
	    cv::FileStorage file;
	...
	const vector<DataPoint> &training_data, const TrainingParameters &tp)
{
	...
	/*
    if (process_rank==0){
        string filename = "main_train_test_init_shapes_rank"+to_string(process_rank)+".yml";
        cv::FileStorage file;
        file.open(filename, cv::FileStorage::WRITE);
...
vector<DataPoint> ArgumentData(const vector<DataPoint> &training_data, int factor)
{
	...
	/*
	if (process_rank==0){
	    string filename = "main_train_ArgumentData.yml";
	    cv::FileStorage file;
	    file.open(filename, cv::FileStorage::WRITE);
	    for (int i = 0; i < result.size(); i++){
          file << "image" << result[i].image;
...
void TrainModel(const vector<DataPoint> &training_data, const TrainingParameters &tp)
{
    ...
    /*
    vector<cv::Point2d> mean_shape0 = MeanShape(shapes, tp);
    {
        string filename = "main_train_meanshape_rank"+to_string(process_rank)+".yml";
        cv::FileStorage file;
        file.open(filename, cv::FileStorage::WRITE);
        file << "meanshape" << mean_shape0;
        file.release();
    }  
    */
    vector<cv::Point2d> mean_shape = MeanShape_MPI(shapes, tp);
    
	vector<vector<cv::Point2d>> test_init_shapes = 
		CreateTestInitShapes_MPI(training_data, tp);    
    /*
	vector<DataPoint> argumented_training_data0 = 
		ArgumentData(training_data, tp.ArgumentDataFactor);
    */
	vector<DataPoint> argumented_training_data = 
		ArgumentData_MPI(training_data, tp.ArgumentDataFactor);
	...
	stage_regressors[0].Regress(mean_shape, &normalized_targets, 
...
int main(int argc, char *argv[])
{
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &process_cnt);
    {
    	...
    	{
    	   {
        	    printf("enter train bracket");
        		TrainingParameters tp = ReadParameters(argv[1]);
        
        		cout << "Training begin." << endl;
        		//vector<DataPoint> training_data0 = GetTrainingData(tp);
        		vector<DataPoint> training_data = GetTrainingData_MPI(tp);
