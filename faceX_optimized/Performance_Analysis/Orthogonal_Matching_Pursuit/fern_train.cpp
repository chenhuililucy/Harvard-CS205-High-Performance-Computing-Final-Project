	...
	cv::Mat pixels_val, cv::Mat pixels_cov)
{
	...
	for (int i = 0; i < training_parameters.F; ++i)
	{
		...
		double Y_proj_cov = CovarianceZY1(Y_proj.ptr<double>(0), 
			Y_proj.ptr<double>(0), Y_proj.total());	// Use AVXCovariance if you want.
		/*double Y_proj_cov = CovarianceZYISPC(Y_proj.ptr<double>(0), 
		...
		{
			Y_pixels_cov[j] = CovarianceZY1(Y_proj.ptr<double>(0), 
				pixels_val.ptr<double>(j), Y_proj.total());	// Use AVXCovariance if you want.
			/*Y_pixels_cov[j] = CovarianceZYISPC(Y_proj.ptr<double>(0), 
