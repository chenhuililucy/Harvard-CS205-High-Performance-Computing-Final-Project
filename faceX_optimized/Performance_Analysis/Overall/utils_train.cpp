...
std::vector<std::pair<int, double>> OMP_Eigen(cv::Mat x, cv::Mat base, int coeff_count)
{
    ...
    for (int i = 0; i < coeff_count; ++i)
    {
        ...
        for (int j = 0; j < base.cols; ++j)
        {
            ...
            double current_value = (column.transpose() * residual).cwiseAbs().sum();
        ...
        result.emplace_back(max_index, 0);
        Eigen::MatrixXd sparse_base(base.rows, result.size());

        for (int j = 0; j < result.size(); ++j)
            baseEigen.col(result[j].first).swap(sparse_base.col(j));

        Eigen::MatrixXd lhs = sparse_base.transpose() * sparse_base;
        Eigen::MatrixXd rhs = sparse_base.transpose() * residual;

        if (lhs.cols() != rhs.rows())
        {
            // Adjust the dimensions if necessary
            lhs.conservativeResize(result.size(), result.size());
            rhs.conservativeResize(result.size(), 1);
        }

        Eigen::MatrixXd beta = lhs.fullPivLu().solve(rhs);
