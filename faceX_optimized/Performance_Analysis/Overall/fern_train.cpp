...
char *mpibuf = new char[bsize];
MPI_Buffer_attach(mpibuf, bsize);
*/
...
//	double buf[training_parameters.F*6];
//	double sbuf[f_workload[process_rank]*2];
	double buf[training_parameters.F * 3];
	double sbuf[training_parameters.F * 3];
//if(isroot) cout<<"Start select feature\n";
...
//if(isroot) cout<<Y_proj.ptr<double>(0)[10]<<" "<<pixels_val.ptr<double>(0)[10]<<"Start 1\n";
		...
		double max_corr = -1;
        int ind1=0, ind2=0;
//if(isroot) cout<<"Start 2\n";
#pragma omp parallel
{
		double t_max_corr = -1;
		int idx1=0, idx2=0;
//cout<<Y_pixels_cov[0]<<" "<<Y_pixels_cov[1]<<"\n";
...
//if(isroot) cout<<ind1<<" "<<ind2<<endl;
/*//FIXME
if(process_rank==1){sbuf[0]=(unsigned char)124; sbuf[4]=(unsigned char)22;}
if(process_rank==2){sbuf[0]=(unsigned char)77; sbuf[4]=(unsigned char)40;}
if(process_rank==3){sbuf[0]=(unsigned char)22; sbuf[4]=(unsigned char)100;}
*/
...
//cout<<"before allocate buf\n";
	double* sendbuf = (double*) calloc(buf_size, sizeof(double));
	double* recvbuf = (double*) calloc(buf_size, sizeof(double));
//	double sendbuf[buf_size];
//	double recvbuf[buf_size];
//if(isroot) cout<<sendbuf[0]<<"before regress\n";

//	shared_ptr<double> sendbuf = (double*)calloc(buf_size, sizeof(double));
//	double *recvbuf = (double*)calloc(buf_size, sizeof(double));
//for(int i=0; i<33;i++) cout<<sendbuf[i]<<" ";
//cout<<endl;
...
#pragma omp parallel for schedule(static) reduction(+: sendbuf[:buf_size])
 ...
 // if(mask==2) cout<<process_rank<<" "<<i<<" "<<pixels_val.at<double>(features_index[1].first, i)<<" "<<pixels_val.at<double>(features_index[1].second,i)<<endl;
...
/*for(int i=0; i<33;i++) cout<<sendbuf[i]<<" ";
cout<<endl;
for(int i=0; i<33;i++) cout<<recvbuf[i]<<" ";
*/
	for (int i = 0; i < outputs_count; ++i)
	{
//cout<<recvbuf[i*block_size+block_size-1]<<" "<<sendbuf[10]<<endl;
...
/*if(isroot)
for(int i=0; i<outputs_count;i++){
cout<<outputs[i][0].x<<" "<<outputs[i][0].y<<endl;
}
for(int i=0; i<training_parameters.F;i++){
cout<<features_index[i].first<<" "<<features_index[i].second<<endl;
}
*/
//const double t2 = MPI_Wtime();
//logtime_for_MPI(t2-t1,"Fern_train"); 
//if(isroot) cout<<(*targets)[0].size()<<"\n";
//MPI_Buffer_detach(mpibuf,&bsize);
//if(isroot) cout<<"before 1\n";
//delete[] mpibuf;
free(sendbuf);
free(recvbuf);
sendbuf=NULL;
recvbuf=NULL;
//if(isroot) cout<<"before 2\n";
//delete[] sendbuf;
//if(isroot) cout<<"before 3\n";
//delete[] recvbuf;
//if(isroot) cout<<"before 4\n";
//delete[] buf;
//if(isroot) cout<<"before 5\n";
//delete[] sbuf;
}
...
void FernTrain::Regress_seq(vector<vector<cv::Point2d>> *targets, 
	cv::Mat pixels_val, cv::Mat pixels_cov)
{
	cv::Mat Y(targets->size(), (*targets)[0].size() * 2, CV_64FC1);
	for (int i = 0; i < Y.rows; ++i)
