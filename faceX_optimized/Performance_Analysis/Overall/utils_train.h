...
Transform ProcrustesV(const std::vector<cv::Point2d> &x,
	const std::vector<cv::Point2d> &y);
...
std::vector<std::pair<int, double>> OMP_MPI(cv::Mat x, cv::Mat base, int coeff_count);

std::vector<std::pair<int, double>> OMP_sub_routine(cv::Mat x, cv::Mat base, int coeff_count);
