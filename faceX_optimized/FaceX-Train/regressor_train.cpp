...
//	const double begin_time = MPI_Wtime(); 
	...
	cv::Mat pixels_val_buffer(training_data.size(), training_parameters_.P, CV_64FC1);


	for (int i = 0; i < size; i++)
	{
		int local_start = i * (pixels_val.cols / size);
		int local_end = (i == size - 1) ? pixels_val.cols : (i + 1) * (pixels_val.cols / size);
		int local_counts = (local_end - local_start)*training_parameters_.P;

		displs[i] = training_parameters_.P*local_start;
		send_counts[i] = local_counts;
	}

	for (int i = start; i < end; ++i) // fill in pixels_val with the values of each pixel offset for each training image
	{
		Transform t = ProcrustesV(training_data[i].init_shape, mean_shape); // calculate the transformation from the initial shape to the mean shape
...
#pragma omp parallel for schedule(static)
		for (int j = 0; j < training_parameters_.P; ++j)
		{
			...
			{
				pixels_val_buffer.at<double>(i, j) =    // Store the pixel value in pixels_val matrix
					training_data[i].image.at<uchar>(pixel_pos);
			}
			else
				pixels_val_buffer.at<double>(i, j) = 0;  // If the pixel position is outside the image, set the pixel value to 0
}
	}
	double* sendbuffer = (double*)pixels_val_buffer.data;
	MPI_Allgatherv(sendbuffer + displs[rank], send_counts[rank], MPI_DOUBLE,
		sendbuffer, send_counts, displs,
		MPI_DOUBLE, MPI_COMM_WORLD);
#pragma omp parallel for schedule(static)
	for (int i = 0; i < pixels_val.cols; i++)
	{
		for (int j = 0; j < training_parameters_.P; ++j)
			pixels_val.at<double>(j, i) = pixels_val_buffer.at<double>(i, j);
...
//	logtime_for_MPI(end_time-begin_time, "Regress-barrier");
	...
	const double end_time = MPI_Wtime(); 
	logtime_for_MPI(end_time-begin_time, "Outer Layer Regression");
...
void RegressorTrain::CompressFerns() // This function compresses the ferns by selecting a smaller base of outputs and finding the indices of the fern outputs that are closest to the base using orthogonal matching pursuit (OMP).
{
	...
	for (int i = 0; i < training_parameters_.K; ++i) // For each fern, find the indices of the closest outputs to the base using OMP.
	{
		...
		// changed to experiment with effect
			ferns_[i].outputs_mini.push_back(OMP_Eigen(output_mat, base_, training_parameters_.Q)); // Compress the output matrix 
		}
	}
	...
	const DataPoint &data) const
{
	...
	Transform t = ProcrustesV(data.init_shape, mean_shape); // Align the initial shape of the test image to the mean shape using Procrustes analysis.
