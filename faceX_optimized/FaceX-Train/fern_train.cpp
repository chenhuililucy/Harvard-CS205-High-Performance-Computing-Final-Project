...
This class implements Fern regressor that is used as the bottom-level 
regressor in FaceX project. Each Fern is fitted in the Fern::Regress 
function. Given training images, each Fern uses covariance to select pairs
of pixels and use their difference as features. It is fitted to the residual 
of previous Ferns.Once fitted, Fern::Apply predicts the correction added to
previous predicted landmarks to make them closer to true landmarks.
*/
	...
	cv::Mat pixels_val, cv::Mat pixels_cov)
{
	...
	// Calculate the workload for all processes 
	...
	// Initialize target array, calculate covariance
	...
	// Begin of feature selection loop
...
// Select pixel pairs with largest correlation
#pragma omp parallel
{
		double t_max_corr = -1;
		int idx1=0, idx2=0;
...
#pragma omp critical
{
		...
		// Calculate difference threshold for the selected pixel pair
		// Utilize AVX2 vector to speedup	
...
#pragma omp atomic update, compare
	...
	} // End of feature selection loop

	// Communicate selected features to other ranks
	MPI_Allgatherv(&sbuf[0], f_workload[process_rank],
		MPI_DOUBLE, &buf[0], f_workload, f_offset, MPI_DOUBLE,MPI_COMM_WORLD);

	// Each process should have the same F features at this point
	...
	// Buffers for regression result communication
...
// Fit to residual of previous landmarks 
#pragma omp parallel for schedule(static) reduction(+: sendbuf[:buf_size])
	...
	// Communicate and aggregate regression results
